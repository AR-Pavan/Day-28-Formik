...
export default function EditUser() {
    ...
    async function getData() {
        try {
            const data = await fetch(`https://631d8e83cc652771a4876ead.mockapi.io/users/${params.id}`).then((res) => res.json());
            setUserData(data);
        } catch (err) {
    ...
    const handleSave = (e) => {
        ...
        async function postData() {
            const newUser = await fetch(`https://631d8e83cc652771a4876ead.mockapi.io/users/${params.id}`, {
                method: "PUT",
                headers: {
